...
def excel_to_df(excel_sheet):
    ...
    df_PL = df_PL.T
    df_PL.columns = list(map(capitalizeWords, df_PL.columns))
    ...
    df_BS = df_BS.T
    df_BS.columns = list(map(capitalizeWords, df_BS.columns))
    df_BS.drop(["Assets","Liabilities"], axis = 1, inplace = True)
    ...
    df_CF = df_CF.T
    df_CF.columns = list(map(capitalizeWords, df_CF.columns))
...
def capitalizeWords(column_name):
   col_name = column_name.split(" ");
   col_name = list(map(str.capitalize, col_name));
